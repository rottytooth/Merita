{
  ...
  const movements = {
    ...
    "mix": "blend(abs(sin(time))/2 + 1/2);",
    "stir": "blend(abs(sin(time))/2 + 1/2);",
    "add": "displace(sin(time)/2,cos(time)/5,0);",
    "cool": "displace(sin(time)/3,cos(time)/4,0);",
    "return": "reset();"
  };
...
//      %MOVEMENT%
  ...
  `, keywords: ["pimiento","pimientos","pimiento--olive","pimiento--olives"]};
...
Program = t:Title newline ing:IngredientBlock newline ins:InstructionBlock _?
{
  ...
  for(let p = 0; p < ing.length; p++) {
    if (ing[p].number)
      ing_sum += ing[p].number;
  }

  code = code + boxes(ing_sum);
